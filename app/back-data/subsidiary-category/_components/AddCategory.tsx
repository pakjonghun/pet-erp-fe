...
const CreateCategoryModal: FC<Props> = ({ open, onClose }) => {
  ...
  const onSubmit = (values: CreateCategoryForm) => {
    createCategory({
      ...
      onCompleted: () => {
        snackMessage({ message: '부자재분류 등록이 완료되었습니다.', severity: 'success' });
        handleClose();
      },
      onError: (err) => {
        const message = err.message;
        snackMessage({ message: message ?? '부자재분류 등록이 실패했습니다.', severity: 'error' });
      },
    });
  };
  ...
  return (
    <BaseModal open={open} onClose={handleClose}>
      ...
      <Typography sx={{ mb: 3 }}>새로운 부자재분류를 등록합니다.</Typography>
